...
from .utils import train_ddh
from .losses import total_loss
import numpy as np
import torch


def last(t): return np.array([t_[-1] for t_ in t], dtype=float)
...
class DynamicDeepHit(DeepRecurrentSurvivalMachines):
    ...
    def discretize(self, t, split, split_time=None):
        ...
        t_discretized = np.array([np.digitize(np.asarray(t_, dtype=np.float64),
                                 split_time, right=True) - 1 for t_ in t], dtype=object)
    ...
    def predict_survival(self, x, t, risk=1, all_step=False, fill_from_last=False, bs=100):
        ...
        if not isinstance(t, (list, np.ndarray)):
            t = [t]
        t_discretized, _ = self.discretize([t], self.split, self.split_time) 
        t = t_discretized[0] 
        t = np.asarray(t, dtype=int)
        t = np.clip(t, 0, self.split - 1) 
        ...
        if self.fitted:
            ...
            for j in range(batches):
                ...
                cumsum_f = torch.cumsum(f[int(risk) - 1], dim=1)
                scores.append(cumsum_f[:, t].detach().numpy())
            predictions = 1 - np.concatenate(scores, axis=0)
            if all_step:
