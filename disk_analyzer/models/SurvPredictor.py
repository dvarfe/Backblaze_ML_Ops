...
from .DLClassifier import ClassifierArchitecture  # maybe move to other module
from ..stages import ModelScorer
...
def ibs_like_loss(surv_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    """MSE between predicted survival and true survival, averaged over all elements.

    Args:
        surv_pred: Predicted survival probabilities, shape (B, T).
        y_true: Ground-truth survival values, shape (B, T).
...
def ibs_remain_torch(survival_train, survival_test, estimate, times: torch.Tensor, axis: int = -1):
    ...
    if estimate.ndim == 1 and times.numel() == 1:
        estimate = estimate.reshape(-1, 1)

    est = torch.where(torch.isfinite(estimate), estimate, torch.zeros_like(estimate))
    ...
    times = times.to(device=device, dtype=torch.float64)

    n_samples, n_times = est.shape
    if times.numel() != n_times:
        raise ValueError("length of times must match estimate.shape[1]")

    est = est.to(dtype=torch.float64)

    estim_before = est.pow(2) * test_event.view(-1, 1)
    estim_after = (1.0 - est).pow(2)

    times_row = times.view(1, -1)           # (1, n_times)
    test_time_col = test_time.view(-1, 1)        # (n_samples, 1)
    mask_before = test_time_col < times_row    # (n_samples, n_times), константа

    brier_matrix = torch.where(mask_before, estim_before, estim_after)

    ones = torch.ones(n_samples, n_times, dtype=torch.float64, device=device)
    N_per_time = torch.where(
        mask_before,
        test_event.view(-1, 1).expand(n_samples, n_times),
        ones
    ).sum(dim=0)  # (n_times,)

    inv_N = torch.where(N_per_time > 0, 1.0 / N_per_time, torch.zeros_like(N_per_time))

    time_diff = (times[-1] - times[0]).item() if times[-1] > times[0] else 1.0

    if axis == -1:
        bs_per_time = inv_N * brier_matrix.sum(dim=0)
        return torch.trapezoid(bs_per_time, times) / time_diff

    elif axis == 0:
        return torch.trapezoid(brier_matrix, times, dim=1) / time_diff

    elif axis == 1:
        return inv_N * brier_matrix.sum(dim=0)
