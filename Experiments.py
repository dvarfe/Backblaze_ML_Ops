...
METHODS = ['LogReg_Classic']
alpha_grid = np.logspace(-3, 2, 6)
l1_grid = np.logspace(-3, 2, 6)
        ...
        for method in METHODS:
            for alpha in alpha_grid:
                for l1 in l1_grid:
                    ...
                    model = SKLClassifier(SGDClassifier(loss='log_loss', l1_ratio=l1, alpha=alpha, warm_start=True))
                    # model = SKLClassifier(LogisticRegression(C = C, warm_start=True))
                    try:
                        model.fit(dl_train)
