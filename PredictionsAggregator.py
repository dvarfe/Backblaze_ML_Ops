...
class PredictionsAggregator():
    ...
    def __init__(self, mode, weight=None):
        ...
        if (not mode in ['n_dist', 't_dist', 'geom', 'prob_dist', 'last', 'first', 'scaled_sum']):
    ...
    def n_dist_agg(self, X):
        ...
        time_columns = result_df.columns[(result_df.columns != self.id_col) & (result_df.columns != 'weights') & (result_df.columns != self.time_col)].tolist()
        
        # Домножаем каждую строку на соответствующий вес
        result_df.loc[:, time_columns] = result_df.loc[:, time_columns].multiply(result_df['weights'], axis=0).astype('float32')
    ...
    def t_dist_agg(self, X):
        ...
        # Оставляем только колонки, которые отвечают за вероятности
        time_columns = X_copy.columns[(X_copy.columns != self.id_col) & (X_copy.columns != self.time_col) & (X_copy.columns != 'weights')].tolist()
    ...
    def geom_agg(self, X):
        ...
        result_df.loc[:, time_columns] = result_df.loc[:, time_columns].mul(result_df['weights'], axis = 'rows').astype('float32')
        result_df = result_df.groupby(self.id_col)[time_columns].sum().div(result_df.groupby(self.id_col)['weights'].sum(), axis='rows')
